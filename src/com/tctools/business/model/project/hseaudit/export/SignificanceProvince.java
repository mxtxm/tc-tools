...
public class SignificanceProvince extends ExportCommon {
    ...
    private void toExcel(
        ...
        try (Workbook wb = new XSSFWorkbook()) {
            Sheet sheet = wb.createSheet("Critical & Major per province");
            ...
            int r = 1;
            for (Map.Entry<String, ProvinceStatistics> entry : result.statistics.entrySet()) {
                int c = 0;
                Row row = sheet.createRow(++r);
                setContractor(wb, row, c, entry.getKey());

                for (ProvinceOrder provinceOrder : result.provinceOrdered) {
                    ProvinceStatistics s = entry.getValue();
        ...
        for (Province province : Services.get(ServiceDtoCache.class).getList(Province.class)) {
            if (provinceIds != null && !provinceIds.isEmpty()) {
                if (provinceIds.contains(province.id)) {
                    result.statistics.put(province.name.get(LANG), new ProvinceStatistics());
                }
            } else {
                result.statistics.put(province.name.get(LANG), new ProvinceStatistics());
            }
    ...
    private static class Result {
        ...
        public Map<String, ProvinceStatistics> statistics = new LinkedHashMap<>(50);
        ...
        public void setValues() {
            ...
            for (Map.Entry<String, ProvinceStatistics> entry : statistics.entrySet()) {
                String provinceName = entry.getKey();
                ProvinceStatistics statistics = entry.getValue();
                ...
                provinceOrder.total += statistics.auditCount;
    ...
    private static class ProvinceStatistics {
